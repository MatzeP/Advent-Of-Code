...
;~ 6-6,4-6
	...
	If ((Number($ElfOne[1]) <= Number($ElfTwo[1])) And (Number($ElfTwo[1]) <= Number($ElfOne[2]))) Then
		$Summe+=1
;~ 		ConsoleWrite($Input[$i]&@CRLF)
	ElseIf ((Number($ElfOne[1]) >= Number($ElfTwo[1])) And (Number($ElfOne[2]) <= Number($ElfTwo[2]))) Then
		$Summe+=1
;~ 		ConsoleWrite($Input[$i]&@CRLF)
	ElseIf ((Number($ElfOne[1]) <= Number($ElfTwo[2])) And (Number($ElfOne[2]) >= Number($ElfTwo[1]))) Then
		$Summe+=1
;~ 		ConsoleWrite($Input[$i]&@CRLF)
	Else
;~ 		ConsoleWrite($Input[$i]&@CRLF)
