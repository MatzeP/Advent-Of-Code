...
For $i=1 To $StrigInput[0]
	...
	If ((Number(StringLen($StringBuffer)) > 14)) Then
		$MarkerString=StringRight($StringBuffer,14)
...
MsgBox(0,"Day-6_StarTwo",$MarkerPos) ;3708
ClipPut($MarkerPos)
...
Func CheckIfNotDouble($StringToCheck)
	...
	For $i=1 to $Split[0]
		Local $searchResult=_ArraySearch($Split,$Split[$i])
		ConsoleWrite($i&"/"&$searchResult&@CRLF)
